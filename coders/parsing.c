...
static int	parse_values(t_args *req, char **argv)
{
	...
	{
		printf("Error: invalid numeric argument(s).%s",
...
static int	is_valid_config(t_args *req, char *scheduler)
{
	...
	{
		printf("Error: invalid shedual argument,%s'%s'\n",
			" expected 'fifo' or 'edf' got ", scheduler);
		return (0);
	}
	return (1);
}
